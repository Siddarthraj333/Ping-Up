...
// find users using name location email name
...
export const sendConnectionRequest = async (req, res) => {
  try {
    ...
    // check if user has sent more than 20 connection requests in the last 24 
    // hours
...
export const getUserConnections = async (req, res) => {
  try {
    ...
    const pendingConnections = (await Connection.find({to_user_id: userId,
      status: 'pending'}).populate('from_user_id')).map(connection=>connection.
        from_user_id)
